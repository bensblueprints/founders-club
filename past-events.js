// ========================================
// FOUNDERS VIETNAM - Past Events Page
// ========================================
...
function loadFeaturedEvent(eventId) {
    ...
    // Update stats
    document.getElementById('attendeeCount').textContent = event.stats.totalAttendees;

    // Load photos if available
    loadEventPhotos(eventId);

    // Handle attendees section based on auth state
    loadAttendeesSection(eventId);
}

function loadEventPhotos(eventId) {
    const photos = Events.getEventPhotos(eventId);
        ...
        galleryGrid.innerHTML = `
            <div class="gallery-placeholder">
                <svg width="48" height="48" viewBox="0 0 48 48" fill="none">
                    ...
                    <path d="M4 32L14 22L22 30L32 18L44 32" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
                <p>Photos coming soon</p>
                <span>Check back after the event photos are uploaded</span>
            </div>
        `;
        return;
    }
    ...
    galleryGrid.innerHTML = photos.map((photo, index) => `
        <div class="gallery-item" onclick="openPhotoViewer(${index})">
            <img src="${photo}" alt="Event photo ${index + 1}">
        </div>
    `).join('');
}

function loadAttendeesSection(eventId) {
    const lockedSection = document.getElementById('attendeesLocked');
    const restrictedSection = document.getElementById('attendeesRestricted');
    ...
    const userAttended = Events.didUserAttendEvent(currentUser.id, eventId);

    if (!userAttended) {
        // Logged in but didn't attend
    ...
    // User attended - show attendees
    lockedSection.style.display = 'none';
    restrictedSection.style.display = 'none';
    attendeesGrid.style.display = 'grid';

    const attendees = Events.getEventAttendees(eventId);
    renderAttendees(attendees, eventId, currentUser.id);
}

function renderAttendees(attendees, eventId, currentUserId) {
    const grid = document.getElementById('attendeesGrid');
    
    // Filter out current user from list
    const otherAttendees = attendees.filter(a => a.id !== currentUserId);

    if (otherAttendees.length === 0) {
        grid.innerHTML = `
            <div class="no-attendees">
                <p>No other attendees to display</p>
            </div>
        `;
        return;
    }

    grid.innerHTML = otherAttendees.map(attendee => `
        <div class="attendee-card" onclick="openAttendeeModal(${attendee.id}, '${eventId}')">
            <div class="attendee-avatar">${attendee.firstName[0]}${attendee.lastName[0]}</div>
            <div class="attendee-info">
                <h4 class="attendee-name">${attendee.firstName} ${attendee.lastName}</h4>
                <p class="attendee-role">${attendee.role}</p>
                <p class="attendee-company">${attendee.company}</p>
            </div>
            <span class="attendee-industry">${getIndustryLabel(attendee.industry)}</span>
            <div class="attendee-quick-connect">
                ${attendee.whatsapp ? `<span class="quick-icon whatsapp" title="WhatsApp available">📱</span>` : ''}
                ${attendee.telegram ? `<span class="quick-icon telegram" title="Telegram available">✈️</span>` : ''}
                ${attendee.zalo ? `<span class="quick-icon zalo" title="Zalo available">💬</span>` : ''}
            </div>
        </div>
    `).join('');
}

function openAttendeeModal(memberId, eventId) {
    const currentUser = Auth.getCurrentUser();
    
    // Double-check permission
    if (!Events.canViewAttendeeProfile(currentUser.id, memberId, eventId)) {
        alert('You must have attended this event to view this profile.');
        return;
    }
...
function initModals() {
    const modal = document.getElementById('attendeeModal');
    const closeBtn = document.getElementById('closeAttendeeModal');
...
// Photo viewer (placeholder for lightbox)
