...
function EventDetailContent() {
    ...
    return <div className="admin-event-page"><section className="admin-event-hero"><div className="container">
        ...
        <section className="panel admin-event-information"><div className="admin-event-section-heading"><div><span className="eyebrow">Event information</span><h2>Event details</h2></div><Link className="button ghost small" href="/admin?tab=events">Return to event manager</Link></div>{Number(event.reserved_seats||0)>Number(event.max_attendees||0)&&<div className="event-overflow-alert"><b>{Number(event.reserved_seats)-Number(event.max_attendees)} guests over capacity</b><span>Reservations remain open. Capacity is a planning threshold and does not block registration or payment.</span></div>}<div className="admin-event-info-grid"><Field label="Event ID" value={event.id}/><Field label="URL slug" value={`/${event.slug}`}/><Field label="Date" value={formatDate(event.event_date, { weekday:'long' })}/><Field label="Time" value={String(event.event_time || '').slice(0,5)}/><Field label="City / location" value={event.location}/><Field label="Venue" value={event.venue_name}/><Field label="Venue address" value={event.venue_address}/><Field label="Dinner price" value={`$${Number(event.dinner_price || 0).toFixed(2)} USD`}/><Field label="Cruise price" value={event.cruise_price != null ? `$${Number(event.cruise_price).toFixed(2)} USD` : null}/><Field label="Capacity" value={`${event.reserved_seats || 0} reserved / ${event.max_attendees || 0} planning threshold`}/><Field label="Cruise capacity" value={event.max_cruise_spots}/><Field label="Paid seats" value={event.paid_seats || 0}/><Field label="Status" value={event.status}/><Field label="Created" value={event.created_at ? new Date(event.created_at).toLocaleString() : null}/><Field label="Description" value={event.description}/></div></section>
