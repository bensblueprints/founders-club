...
const fallback = [
    { id: 'danang', slug: 'danang-jul-2026', name: 'Founders Dinner · Da Nang', event_date: '2026-07-31', location: 'Da Nang', venue_name: 'FOR YOU STEAKHOUSE', venue_address: 'Lô 1C - 01 Võ Nguyên Giáp, An Hải, Đà Nẵng 550000, Việt Nam', dinner_price: 150, status: 'open', description: 'A phone-free dinner for a carefully selected group of founders.', image_url: '/images/gallery/venue-wide.jpg' },
    ...
    return <div className="event-grid">
        {rows.map((event, index) => <article className="event-card" key={event.id || event.slug}>
            ...
            <div className="event-card-body"><span className="pill">{event.status || (past ? 'completed' : 'open')}</span><h2>{event.name || event.title}</h2><p className="muted">{event.description || 'A curated founder gathering built for meaningful conversations.'}</p>
                <div className="event-meta"><span><CalendarDays size={16} /> {formatDate(event.event_date || event.date, { weekday: 'long' })}</span><span><MapPin size={16} /> {event.venue_name || event.location || event.venue || 'Vietnam'}</span><span><UsersRound size={16} /> {event.max_attendees || event.seats || 25} seats · ${Number(event.dinner_price || event.price || 150)}</span></div>
